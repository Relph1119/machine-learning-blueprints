...
X = gft.X
ipos = gft.ipos
X_train, X_test = X[461:], X[:461]
y_train = ipos['$ Chg Open to Close'][461:].map(lambda x: 1 if x>=.25 else 0)
y_test = ipos['$ Chg Open to Close'][:461].map(lambda x: 1 if x>=.25 else 0)
